...
async function trans(text) {
  const translated = await gtrans(text, { to: 'id' }).then(res => res.data);

  console.log(`Word(s) "${translated.sourceText}" has meaning "${translated.translated}"`); 

  if (translated.isCorrected) {
    console.log(`did you mean "${translated.corrected.replace(/\*/g, '')}"?`);
  }
  ...
  console.log(`example: ${translated.examples[Math.floor(Math.random() * translated.examples.length)]}\n`);
